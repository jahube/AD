...
##########################################################################
While ($url -ne $Null) {
    $data = (Invoke-WebRequest -Headers $headers -Uri $url -Proxy "http://proxy-dc.datagroup.local:3128" -ProxyUseDefaultCredentials
     #$data = (Invoke-WebRequest -Headers $headers -Uri $url
) | ConvertFrom-Json
    $lastlogindates += $data.Value | select displayName,userPrincipalName,userType,  @{ N= "lastSignInDateTime" ; E = { $_.signInActivity.lastSignInDateTime } },@{ N= "lastNonInteractiveSignInDateTime" ; E = { $_.signInActivity.lastNonInteractiveSignInDateTime } }
