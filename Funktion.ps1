...
$OU = "OU=$company-Funktion,DC=$company,DC=local"
$prefix = "$company-Funktion"
...
$UPN_LOCAL = $MBXs_domains | where { $_.UPN_Domain -match "$company.local$" }
$SMTP_LOCAL = $MBXs_domains | where { $_.SMTP_Domain -match "$company.local$" }

$MISSING_TARGET = $MBXs_domains | where { !($_.TargetAddress) -or ($_.TargetAddress -notmatch '@') }

$INVALID_SMTP = $MBXs_domains | where { $_.SMTP_Domain -ne "$company.de" }

$INVALID_UPN = $MBXs_domains | where {  $_.UPN_Domain -ne "$company.de" }

$INVALID_MAIL = $MBXs_domains | where { $_.MAIL_Domain -ne "$company.de" }

$path = "C:\Temp"

$datestamp = Get-Date -Format yyyy.MM.dd_HH.MM

$UL_filepath = $path + '\' + $prefix + '_UPN_LOCAL_' + $datestamp + '.CSV'
$SL_filepath = $path + '\' + $prefix + '_SMTP_LOCAL_' + $datestamp + '.CSV'
...
$UPN_LOCAL | Export-Csv -Path $UL_filepath -Delimiter ";" -Encoding UTF8 -NoTypeInformation -Force
$SMTP_LOCAL|Export-Csv -Path $SL_filepath -Delimiter ";" -Encoding UTF8 -NoTypeInformation -Force
